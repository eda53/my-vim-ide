...
" previous nonblank, non-comment line. 0 for not-found.
function! s:VHDLprevline(curn)
    let prevn = prevnonblank(a:curn - 1)
...
function! s:VHDLindent(lnum)
    ...
    if curs =~ '^\s*--'
        ...
        if curs =~ '^\s*--\s' && ps =~ '--'
            ...
            " comment belongs to above code because previous line is not empty while next line is.
            "if (prevnonblank(pn) == pn) && (nextnonblank(curn+1) != (curn+1))
            "    return ind
            "endif
            ...
            endwhile
            let n = indent(nn)
            return n != -1 ? s:VHDLindent(nn) : ind
        endif
    endif
        ...
        while pn > 0
            ...
            if ps !~ '^\s*--' && (t < ind || (t == ind && m == 3))
                ...
                if m < 3 && ps !~? '^\s*\S\+\s*<=[^;]*'.s:ES
                    if (ps =~? s:NC.'\<\%(procedure\|generic\|map\|port\)\>' || ps =~ '^\s*(') && (prevs =~ s:NC.')\s*;' || curs =~? s:NC.'\<\%(procedure\|generic\|map\|port\)\>')
                        let ind = t
                    endif
                    break
                endif
    ...
    if curs =~? s:NC.'\<begin\>' && curs !~? s:NC.'\<process.\+\<begin\>'
