...
"		call s:EditDir(expand("#" . filter(range(1, bufnr("$")), 'buflisted(v:val) && bufwinnr(v:val)!=-1')[0] . ":p:h"))
"		echomsg "current buffer:winbufnr(0)=" . winbufnr(0)
"		call s:EditDir(expand("#" . winbufnr(0) . ":p:h"))
"	end
	let bufnum = WinManagerGetLastEditedFile()
	if bufnum != -1
		call s:EditDir(fnamemodify(bufname(bufnum), ':p:h'))
	else
		call s:EditDir(expand("%:p:h"))
	endif
